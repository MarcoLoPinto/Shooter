class FillViewport {
	...
	resizeCanvas(width,height){
		...
		if( (width/height) < (this.ratio) ){
			...
			this.canvas.style.left = Math.round((width*browserZoomLevel-this.canvas.width)/(2*browserZoomLevel));
	...
	drawGrid(){
		for(var i = 0; i < this.canvas.width; i = i + this.canvas.width/this.unit)
			for(var j = 0; j < this.canvas.height ; j = j + this.canvas.width/this.unit)
				this.ctx.strokeRect(i,j,this.canvas.width/this.unit,this.canvas.width/this.unit);
	}
